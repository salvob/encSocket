...
// large pages.
//
// Modified: Salvo Barbagallo for multipacket
// 2013-01-20 <epromtech@gmail.com>
...
static void setMACs (const byte *mac) {
  EncSocket::copyMac(gPB + ETH_DST_MAC, mac);
  EncSocket::copyMac(gPB + ETH_SRC_MAC, EncSocket::mymac);
}

static void setMACandIPs (const byte *mac, const byte *dst) {
  setMACs(mac);
  EncSocket::copyIp(gPB + IP_DST_P, dst);
  EncSocket::copyIp(gPB + IP_SRC_P, EncSocket::myip);
}
...
static byte eth_type_is_arp_and_my_ip(word len) {
  return len >= 41 && gPB[ETH_TYPE_H_P] == ETHTYPE_ARP_H_V &&
                      ...
                      memcmp(gPB + ETH_ARP_DST_IP_P, EncSocket::myip, 4) == 0;
}
...
static byte eth_type_is_ip_and_my_ip(word len) {
  return len >= 42 && gPB[ETH_TYPE_H_P] == ETHTYPE_IP_H_V &&
                      ...
                      memcmp(gPB + IP_DST_P, EncSocket::myip, 4) == 0;
}
...
static void make_eth_ip() {
  ...
  EncSocket::copyIp(gPB + IP_DST_P, gPB + IP_SRC_P);
  EncSocket::copyIp(gPB + IP_SRC_P, EncSocket::myip);
...
static void make_arp_answer_from_request() {
  ...
  EncSocket::copyMac(gPB + ETH_ARP_DST_MAC_P, gPB + ETH_ARP_SRC_MAC_P);
  EncSocket::copyMac(gPB + ETH_ARP_SRC_MAC_P, EncSocket::mymac);
  EncSocket::copyIp(gPB + ETH_ARP_DST_IP_P, gPB + ETH_ARP_SRC_IP_P);
  EncSocket::copyIp(gPB + ETH_ARP_SRC_IP_P, EncSocket::myip);
  EncSocket::packetSend(42); 
}
...
static void make_echo_reply_from_request(word len) {
  ...
  EncSocket::packetSend(len);
}

void EncSocket::makeUdpReply (char *data,byte datalen,word port) {
  if (datalen>220)
      datalen = 220;
...
static void make_tcp_synack_from_syn() { 
  ...
  EncSocket::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+4+ETH_HEADER_LEN);
}
...
static void make_tcp_ack_from_any(int16_t datlentoack,byte addflags) {
  ...
  EncSocket::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN);
}




static void make_tcp_ack_with_data_noflags_delay(word dlen, uint8_t rt, uint8_t dlay){
  ...
  gPB[TCP_CHECKSUM_H_P] = 0;
  gPB[TCP_CHECKSUM_L_P] = 0;
  gPB[TCP_WIN_SIZE] = 0x3; // 1024=0x400, 1280=0x500 2048=0x800 768=0x300
  gPB[TCP_WIN_SIZE+1] = 0;
  fill_checksum(TCP_CHECKSUM_H_P, IP_SRC_P, 8+TCP_HEADER_LEN_PLAIN+dlen,2);
  for(uint8_t i = 0; i < rt; i++){
	delay(dlay--);
  	EncSocket::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlen+ETH_HEADER_LEN);
...
void EncSocket::httpServerReply (word dlen) {
...
static void get_seq() { 
SEQ =((gPB[TCP_SEQ_H_P]*256+gPB[TCP_SEQ_H_P+1])*256+gPB[TCP_SEQ_H_P+2])*256+gPB[TCP_SEQ_H_P+3];
}


static void set_lastAck() { 
	for(uint8_t i = 0; i < 54; i++) enc.buffer[i] = backup[i];
}


//set the correct sequence number and calculate the next with the lenght of current packet
static void set_seq(word dlen) { 
...
static void waitAck(word dlen){
	boolean find = true;
	unsigned long seqAck;
	unsigned long seq;
	//Serial.print("Attendo Ack...");
	uint8_t counter=0;
	uint8_t resend = 0;
	uint16_t lastmillis;
	uint16_t millis = 0;
	...
	while(find){
		millis++;
		if(EncSocket::packetReceived(0xa) > 0){
		//	Serial.print("SRC:");Serial.print(enc.buffer[6],HEX);Serial.println(enc.buffer[7],HEX);
		//	Serial.print("SEQ:");for(uint8_t i = 0; i < 4; i++)Serial.print(enc.buffer[TCP_SEQ_H_P+i],HEX);//Serial.println();
		//	Serial.print(" | ACK:");for(uint8_t i = 0; i < 4; i++)Serial.print(enc.buffer[TCP_SEQACK_H_P+i],HEX);//Serial.println();
			
			seq  = backup[TCP_SEQ_H_P] << 24 | backup[TCP_SEQ_H_P+1] << 16 | backup[TCP_SEQ_H_P+2] << 8 | backup[TCP_SEQ_H_P+3];
			seqAck = enc.buffer[TCP_SEQACK_H_P] << 24 | enc.buffer[TCP_SEQACK_H_P+1] << 16 | enc.buffer[TCP_SEQACK_H_P+2] << 8 | enc.buffer[TCP_SEQACK_H_P+3];
		...
		//		Serial.print("..Eccolo");
				...
				set_lastAck();
		 		make_tcp_ack_with_data_noflags_delay(dlen,1,1); // send data
				delay(1);
				counter = 0;
		//		Serial.print("..Resend");
				resend++;
	...
	//		Serial.println();
		...
		else if((millis-lastmillis) > 10){
			set_lastAck();
	 		make_tcp_ack_with_data_noflags_delay(dlen,1,1);
			delay(1);
	//		Serial.println("REinvioTEmpoScaduto");
			lastmillis = millis;
			resend++;
		}
	
		if(resend > 9) find = false;
	}
...
void EncSocket::httpServerReplyAck () {
	make_tcp_ack_from_any(info_data_len,0); // send ack for http get
...
void EncSocket::httpServerReply_with_flags (word dlen , byte flags) {
	
	byte fl = TCP_FLAGS_ACK_V|TCP_FLAGS_FIN_V;
	...
	for(uint8_t i = 0; i < 54; i++) backup[i] = enc.buffer[i];
	waitAck(dlen);
...
void EncSocket::clientIcmpRequest(const byte *destip) {
  ...
  gPB[ICMP_IDENT_L_P] = EncSocket::myip[3]; // last byte of my IP
...
void EncSocket::ntpRequest (byte *ntpip,byte srcport) {
...
byte EncSocket::ntpProcessAnswer (uint32_t *time,byte dstport_l) {
...
void EncSocket::udpPrepare (word sport, byte *dip, word dport) {
  setMACandIPs(gwmacaddr, dip);
  // see http://tldp.org/HOWTO/Multicast-HOWTO-2.html
  // multicast or broadcast address, https://github.com/jcw/EncSocket/issues/59
  if ((dip[0] & 0xF0) == 0xE0 || *((long*) dip) == 0xFFFFFFFF)
    EncSocket::copyMac(gPB + ETH_DST_MAC, allOnes);
  gPB[ETH_TYPE_H_P] = ETHTYPE_IP_H_V;
  gPB[ETH_TYPE_L_P] = ETHTYPE_IP_L_V;
  memcpy_P(gPB + IP_P,iphdr,9);
  gPB[IP_TOTLEN_H_P] = 0;
...
void EncSocket::udpTransmit (word datalen) {
  gPB[IP_TOTLEN_H_P] = (IP_HEADER_LEN+UDP_HEADER_LEN+datalen) >> 8;
  gPB[IP_TOTLEN_L_P] = IP_HEADER_LEN+UDP_HEADER_LEN+datalen;
  fill_ip_hdr_checksum();
  gPB[UDP_LEN_H_P] = (UDP_HEADER_LEN+datalen) >>8;
...
void EncSocket::sendUdp (char *data,byte datalen,word sport, byte *dip, word dport) {
...
void EncSocket::sendWol (byte *wolmac) {
...
static void client_arp_whohas(byte *ip_we_search) {
  ...
  EncSocket::copyMac(gPB + ETH_ARP_SRC_MAC_P, EncSocket::mymac);
  EncSocket::copyIp(gPB + ETH_ARP_DST_IP_P, ip_we_search);
  EncSocket::copyIp(gPB + ETH_ARP_SRC_IP_P, EncSocket::myip);
  waitgwmac |= WGW_ACCEPT_ARP_REPLY;
  EncSocket::packetSend(42);
}

byte EncSocket::clientWaitingGw () {
  return !(waitgwmac & WGW_HAVE_GW_MAC);
}

static byte client_store_gw_mac() {
  if (memcmp(gPB + ETH_ARP_SRC_IP_P, EncSocket::gwip, 4) != 0)
    return 0;
  EncSocket::copyMac(gwmacaddr, gPB + ETH_ARP_SRC_MAC_P);
  return 1;
}
...
void EncSocket::setGwIp (const byte *gwipaddr) {
...
static void client_syn(byte srcport,byte dstport_h,byte dstport_l) {
  setMACandIPs(gwmacaddr, EncSocket::hisip);
  gPB[ETH_TYPE_H_P] = ETHTYPE_IP_H_V;
  gPB[ETH_TYPE_L_P] = ETHTYPE_IP_L_V;
  memcpy_P(gPB + IP_P,iphdr,9);
  ...
  EncSocket::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN+4);
}

byte EncSocket::clientTcpReq (byte (*result_cb)(byte,byte,word,word),
...
static word www_client_internal_datafill_cb(byte fd) {
  BufferFiller bfill = EncSocket::tcpOffset();
  if (fd==www_fd) {
    if (client_postval == 0) {
...
void EncSocket::browseUrl (prog_char *urlbuf, const char *urlbuf_varpart, prog_char *hoststr,void (*callback)(byte,word,word)) {
...
void EncSocket::httpPost (prog_char *urlbuf, prog_char *hoststr, prog_char *additionalheaderline,const char *postval,void (*callback)(byte,word,word)) {
  client_urlbuf = urlbuf;
  client_hoststr = hoststr;
...
static word tcp_datafill_cb(byte fd) {
  ...
  Stash::extract(0, len, EncSocket::tcpOffset());
  Stash::cleanup();
  EncSocket::tcpOffset()[len] = 0;
#if SERIAL
  Serial.print("REQUEST: ");
  Serial.println(len);
  Serial.println((char*) EncSocket::tcpOffset());
#endif
...
static byte tcp_result_cb(byte fd, byte status, word datapos, word datalen) {
  if (status == 0) {
    result_fd = fd; // a valid result has been received, rememer its session id
    result_ptr = (char*) enc.buffer + datapos;
    // result_ptr[datalen] = 0;
  }
  return 1;
}

byte EncSocket::tcpSend () {
  www_fd = clientTcpReq(&tcp_result_cb, &tcp_datafill_cb, hisport);
  return www_fd;
}

const char* EncSocket::tcpReply (byte fd) {
  if (result_fd != fd)
    return 0;
...
void EncSocket::registerPingCallback (void (*callback)(byte *srcip)) {
  icmp_cb = callback;
}

byte EncSocket::packetLoopIcmpCheckReply (const byte *ip_monitoredhost) {
...
word EncSocket::accept(const word port, word plen) {
  word len;
...
word EncSocket::packetLoop (word plen) {
  word len;
...
void EncSocket::persistTcpConnection(bool persist){
