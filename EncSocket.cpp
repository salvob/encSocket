...
// Hence: GPL V2
//
// 2013-01-19 <epromtech@gmail.com>
...
void Stash::load (uint8_t idx, uint8_t blk) {
  if (blk != bufs[idx].bnum) {
    if (idx == 0) {
      enc.copyout(bufs[idx].bnum, bufs[idx].bytes);
      if (blk == bufs[1].bnum)
    ...
    bufs[idx].bnum = blk;
    enc.copyin(bufs[idx].bnum, bufs[idx].bytes);
  }
}
...
uint8_t EncSocket::mymac[6];  // my MAC address
uint8_t EncSocket::myip[4];   // my ip address
uint8_t EncSocket::mymask[4]; // my net mask
uint8_t EncSocket::gwip[4];   // gateway
uint8_t EncSocket::dhcpip[4]; // dhcp server
uint8_t EncSocket::dnsip[4];  // dns server
uint8_t EncSocket::hisip[4];  // dns result
uint16_t EncSocket::hisport = 80; // tcp port to browse to
bool EncSocket::using_dhcp = false;
bool EncSocket::persist_tcp_connection = false;

uint8_t EncSocket::begin (const uint16_t size,
...
bool EncSocket::staticSetup (const uint8_t* my_ip,
                              const uint8_t* gw_ip,
