...
void ENC28J60::packetSend(word len) {
	...
	unsigned long seq  = buffer[TCP_SEQ_H_P] << 24 | buffer[TCP_SEQ_H_P+1] << 16 | buffer[TCP_SEQ_H_P+2] << 8 | buffer[TCP_SEQ_H_P+3];
	unsigned long seqAck  = buffer[TCP_SEQACK_H_P] << 24 | buffer[TCP_SEQACK_H_P+1] << 16 | buffer[TCP_SEQACK_H_P+2] << 8 | buffer[TCP_SEQACK_H_P+3];
	
	if(false){
		Serial.println("Invio pacchetto");
		for(uint8_t i = 54; i < len; i++)Serial.print((char)buffer[i]);
		Serial.println();
		Serial.print("SEQ:");for(uint8_t i = 0; i < 4; i++)Serial.print(buffer[TCP_SEQ_H_P+i],HEX);Serial.println();
		Serial.print("ACK:");for(uint8_t i = 0; i < 4; i++)Serial.print(buffer[TCP_SEQACK_H_P+i],HEX);Serial.println();
		Serial.println();
		Serial.println();
	}
    ...
    if (readRegByte(EPKTCNT) > 0) {
        ...
        if ((header.status & 0x80)==0)
			...
			if(false ){
					...
					Serial.println("Ricevo pacchetto");
					//unsigned long seq  = buffer[TCP_SEQ_H_P] << 24 | buffer[TCP_SEQ_H_P+1] << 16 | buffer[TCP_SEQ_H_P+2] << 8 | buffer[TCP_SEQ_H_P+3];
				//	unsigned long seqAck  = buffer[TCP_SEQACK_H_P] << 24 | buffer[TCP_SEQACK_H_P+1] << 16 | buffer[TCP_SEQACK_H_P+2] << 8 | buffer[TCP_SEQACK_H_P+3];
				//	byte flag = buffer[TCP_FLAGS_P];
				//	seq = seq -44;
					for(uint8_t i = 26; i < 30; i++){Serial.print(buffer[i]);Serial.print(".");}
					Serial.println();
...
word ENC28J60::packetReceived(byte ack) {
    word len = 0;
	word maxlen = 70;
	maxlen = bufferSize;
    if (readRegByte(EPKTCNT) > 0) {
        ...
        if ((header.status & 0x80)==0){
            ...
            readBuf(len, buffer);
			if(buffer[TCP_FLAGS_P] == 0x10 && buffer[TCP_SEQACK_H_P+3] == ack){
					len = 71;
		  }
