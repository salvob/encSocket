...
void EncSocket::copyIp (byte *dst, const byte *src) {
    memcpy(dst, src, 4);
}

void EncSocket::copyMac (byte *dst, const byte *src) {
    memcpy(dst, src, 6);
}

void EncSocket::printIp (const char* msg, const byte *buf) {
    Serial.print(msg);
    EncSocket::printIp(buf);
    Serial.println();
}

void EncSocket::printIp (const __FlashStringHelper *ifsh, const byte *buf) {
	Serial.print(ifsh);
	EncSocket::printIp(buf);
	Serial.println();
}

void EncSocket::printIp (const byte *buf) {
	for (byte i = 0; i < 4; ++i) {
...
byte EncSocket::findKeyVal (const char *str,char *strbuf, byte maxlen,const char *key)
{
    byte found=0;
    byte i=0;
    const char *kp;
...
void EncSocket::urlDecode (char *urlbuf)
{
    char c;
    char *dst = urlbuf;
    while ((c = *urlbuf) != 0) {
...
void EncSocket::urlEncode (char *str,char *urlbuf)
{
    char c;
    while ((c = *str) != 0) {
        if (c == ' '||isalnum(c)){ 
...
byte EncSocket::parseIp (byte *bytestr,char *str)
{
    char *sptr;
    byte i=0;
    sptr=NULL;
...
void EncSocket::makeNetStr (char *resultstr,byte *bytestr,byte len,char separator,byte base)
{
    byte i=0;
    byte j=0;
    while(i<len){
