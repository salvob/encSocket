...
static void send_dhcp_message (void) {
    ...
    EncSocket::udpPrepare(DHCP_DEST_PORT, 
		(dhcpState == DHCP_STATE_BOUND ? EncSocket::dhcpip : allOnes),
		DHCP_SRC_PORT);   // SRC<->DST ??
    
	if (dhcpState != DHCP_STATE_BOUND)
		EncSocket::copyMac(gPB + ETH_DST_MAC, allOnes); //force broadcast mac
	...
	if (dhcpState == DHCP_STATE_BOUND) {
		EncSocket::copyIp(dhcpPtr->ciaddr, EncSocket::myip); 
	}
    EncSocket::copyMac(dhcpPtr->chaddr, EncSocket::mymac);
    ...
    EncSocket::udpTransmit((bufPtr - gPB) - UDP_DATA_P);
}
...
static void process_dhcp_offer (word len) {
    ...
    EncSocket::copyIp(EncSocket::myip, dhcpPtr->yiaddr);
    ...
    do {
        ...
        byte optionLen = *ptr++;
        switch (option) {
            case 1:  EncSocket::copyIp(EncSocket::mymask, ptr);
                     break;
            case 3:  EncSocket::copyIp(EncSocket::gwip, ptr);
                     break;
            case 6:  EncSocket::copyIp(EncSocket::dnsip, ptr);
                     break;
            case 51:
            case 58: leaseTime = 0; // option 58 = Renewal Time, 51 = Lease Time
                     ...
                     break;
            case 54: EncSocket::copyIp(EncSocket::dhcpip, ptr);
                     break;
        }
        ptr += optionLen;
    } while (ptr < gPB + len);
...
void EncSocket::DhcpStateMachine (word len) {
