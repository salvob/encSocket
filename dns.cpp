...
static void dnsRequest (const prog_char *progmem_hostname, bool fromRam) {
  ...
  if (enc.dnsip[0] == 0)
    memset(enc.dnsip, 8, 4); // use 8.8.8.8 Google DNS as default
  enc.udpPrepare((DNSCLIENT_SRC_PORT_H << 8) | dnstid_l,
                                                enc.dnsip, 53);
...
bool EncSocket::dnsLookup (prog_char* name, bool fromRam) {
  word start = millis();
